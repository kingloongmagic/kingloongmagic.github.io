 function st(self, page, product, anchor) {
    ...
    if (searchBox && searchBox.value !== '') {
        ...
        if (productUrl[product]) {
            newHref = productUrl[product] + "?word=" + query;
        }else{
            newHref="/baidu/" + "?word=" + query;
        }
    ...
    $(self).parent().siblings().removeClass('cur');  // 删除其他兄弟元素的样式
...
function focusInput(a) {
    ...
    a.onfocus = function() {
        b.className = "qborder blueQborder";
        if (typeof uigsPB != "undefined") {
            uigsPB(a.getAttribute("id"))
        }
    };
    if (a.getAttribute("ov")) {
        setTimeout(function() {
            if (a.value != a.getAttribute("ov")) {
                a.value = a.getAttribute("ov")
            }
